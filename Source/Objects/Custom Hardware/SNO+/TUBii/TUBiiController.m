...
- (IBAction)DataReadoutChanged:(id)sender {
    BOOL data_readout = ([[sender selectedCell] tag] == 1);
        @try {
            [model setDataReadout:data_readout];
        } @catch(NSException *exception) {
            [self log_error:exception];
            return;
        }
...
- (IBAction)TrigMaskMatchHardware:(id)sender {
    ...
    NSUInteger syncMaskVal = 0xffffff - asyncMask;
...
- (IBAction)ECAEnableChanged:(id)sender {
    BOOL ecal_mode = ([[ECA_EnableButton selectedCell] tag] == 1);
    @try {
        [model setECALMode: ecal_mode];
    } @catch(NSException *exception) {
        [self log_error:exception];
        return;
    }
...
- (void) log_error:(NSException *)exception {
    NSLogColor([NSColor redColor], @"TUBii Error: %@\n",[exception reason]);
}
@end
