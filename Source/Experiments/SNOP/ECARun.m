    ...
    @try{
        for (int step=0; step < eca_pattern_num_steps; step++) {
            ...
            for (int ipoint=0; ipoint<tslope_nsteps; ipoint++) {
                ...
                int current_coarse_delay = [[tslope_delays_coarse objectAtIndex:ipoint] intValue];
                int current_fine_delay = [[tslope_delays_fine objectAtIndex:ipoint] intValue];
                NSLog(@"************************* \n");
                ...
                [aSNOPModel updateEPEDStructWithCoarseDelay: 255 - floor(current_coarse_delay/10.0)
                                                  fineDelay: current_fine_delay
                                             chargePulseAmp: 0x0
                                              pedestalWidth: pedestal_width
                                                    calType: 2 * 10 + (ECA_pattern+1)];
